...
import { useUpdateChurchMutation } from "../../redux/services/ChurchApi";
...
const AdminEditChurchModal: FC<NewDistrictModalProps> = ({ closeModal, data }) => {
    console.log(data)
  const { data: Unassigned } = useFetchUnassignedUserQuery();
  const { data: DistrictList } = useFetchDistrictListQuery();
  const [UpdateChurch, {isLoading}] = useUpdateChurchMutation() 
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  useEffect(() => {
    setValue("church_name", data.church_name)
    setValue("church_date_establish", new Date(data.church_date_establish).toISOString().split('T')[0]);
    setValue("church_address", data.church_address)
    setValue("district_id", data.district_id)
    setValue("pastor_assign", data.pastor_assign)

  },[data, setValue])

  const onSubmitHandler: SubmitHandler<ChurchPayload> = async (value) => {
    await UpdateChurch({data:value, id: data.id})
      .unwrap()
      .then((response) => {
      ...
      <div className=" max-h-[90vh] h-max pb-10 lg:min-w-[400px] w-96 dark:bg-fourth-dark dark:text-white  bg-white  rounded-[10px]  p-4 mx-4 fixed top-0  ">
        ...
        >
          ...
          <div className="w-full mt-[15px]">
            ...
            <TextField
              type="text"
              label={data.head_pastor_full_name}
              select
...
export default AdminEditChurchModal;
